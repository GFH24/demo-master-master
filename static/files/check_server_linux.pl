...
$pre_cmd{55} = "if [ `echo \$SHELL|egrep \"bash|sh\"|wc -l` -ge 1 ]
then
...
";
push(@array_pre_flag, 55);$pre_cmd{54} = "cat /etc/profile|grep -v \"^[[:space:]]*#\"|grep \"HISTFILESIZE\"
cat /etc/profile|grep -v \"^[[:space:]]*#\"|grep \"HISTSIZE\"
";
push(@array_pre_flag, 54);$pre_cmd{53} = "cat /etc/host.conf|grep -v \"^[[:space:]]*#\"|egrep \"order[[:space:]]hosts,bind|multi[[:space:]]on|nospoof[[:space:]]on\"
";
push(@array_pre_flag, 53);$pre_cmd{52} = "cat /proc/sys/net/ipv4/tcp_syncookies
";
push(@array_pre_flag, 52);$pre_cmd{45} = "chkconfig --list
netstat -an
";
push(@array_pre_flag, 45);$pre_cmd{51} = "cat /proc/sys/net/ipv4/conf/*/accept_source_route
";
push(@array_pre_flag, 51);$pre_cmd{50} = "lsattr /var/log/messages 2>/dev/null
";
push(@array_pre_flag, 50);$pre_cmd{49} = "if [ -f /etc/syslog.conf ]
then
...
";
push(@array_pre_flag, 49);$pre_cmd{48} = "redhat_count=0
suse_count=0
result=0
...
";
push(@array_pre_flag, 48);$pre_cmd{1} = "if ([ -f /etc/redhat-release ] && [ -f /etc/pam.d/system-auth ]);
...
";
push(@array_pre_flag, 3);$pre_cmd{4} = "if grep -v \"^[[:space:]]*#\" /etc/ssh/sshd_config|grep -i \"ermitRootLogin no\"
...
suse_version=`cat /etc/SuSE-release 2>/dev/null|grep -i \"VERSION\"|awk '{print \$3}'`
ubuntu_version1=`cat /etc/issue 2>/dev/null|grep -i \"Ubuntu\"|awk '{print \$1}'`
ubuntu_version2=`lsb_release -a 2>/dev/null|grep -i \"Description:\" |awk '{print \$2}'`
if ([ \"x\$suse_version\" = x10 ] || [ \"x\$suse_version\" = 11 ] || [ \"x\$suse_version\" = x11 ] || [ \"\$ubuntu_version1\" = Ubuntu ] || [ \"\$ubuntu_version2\" = Ubuntu ])
then
FILE=/etc/pam.d/common-password
Calculate;
else
...
";
push(@array_pre_flag, 15);$pre_cmd{17} = "cat /etc/profile|grep -v \"^[[:space:]]*#\"|grep \"ulimit[[:space:]]*-S[[:space:]]*-c[[:space:]]*0[[:space:]]*>[[:space:]]*/dev/null[[:space:]]*2>&1\"
cat /etc/security/limits.conf|grep -v \"[[:space:]]*#\"
";
push(@array_pre_flag, 17);$pre_cmd{16} = "chkconfig --list |egrep \"telnet\"
...
unset telnet_status
";
push(@array_pre_flag, 16);$pre_cmd{19} = "ssh_status=`ps -ef|grep \"sshd\"|grep -v \"grep\"|wc -l`;
...
unset ssh_status;
";
push(@array_pre_flag, 19);$pre_cmd{18} = "if [ -f /etc/syslog.conf ];
then
...
";
push(@array_pre_flag, 18);$pre_cmd{21} = "ls -lL /etc/passwd 2>/dev/null
...
";
push(@array_pre_flag, 21);$pre_cmd{20} = "egrep \"^lp:|^sync:|^halt:|^news:|^uucp:|^operator:|^games:|^gopher:|^smmsp:|^nfsnobody:|^nobody:\" /etc/shadow|awk -F: '(\$2!~/\\*LK\\*/) {print \$1\":\"\$2}'
...
";
push(@array_pre_flag, 20);$pre_cmd{23} = "UP_GIDMIN=`(grep -v ^# /etc/login.defs |grep \"^GID_MIN\"|awk '(\$1=\"GID_MIN\") {print \$2}')`
...
";
push(@array_pre_flag, 23);$pre_cmd{22} = "if [ -f /etc/syslog.conf ];
then
syslog=`cat /etc/syslog.conf | grep -v \"^[[:space:]]*#\" | grep \"*.err\\;kern\\.debug\\;daemon\\.notice[[:space:]]*\"|wc -l`;
if [ \$syslog -ge 1 ];
then
...
if [ -f /etc/rsyslog.conf ];
then
rsyslog=`cat /etc/rsyslog.conf | grep -v \"^[[:space:]]*#\" | grep \"*.err\\;kern\\.debug\\;daemon\\.notice[[:space:]]*\"|wc -l`;
if [ \$rsyslog -ge 1 ];
then
...
if [ -s /etc/syslog-ng/syslog-ng.conf ];
then suse_ret0=`cat /etc/syslog-ng/syslog-ng.conf | grep -v \"^[[:space:]]*#\" | grep \"level(err) or facility(kern) and level(debug) or facility(daemon) and level(notice)\"`;
suse_ret1=`cat /etc/syslog-ng/syslog-ng.conf | grep -v \"^[[:space:]]*#\" | grep \"not facility\"|egrep -v \"kern|daemon\"`;
if [ -n \"\$suse_ret0\" ] || [ -n \"\$suse_ret1\" ];
then suse_ret2=`cat /etc/syslog-ng/syslog-ng.conf | grep -v \"^[[:space:]]*#\" | grep 'file(\"/var/log/messages\")'`;
if [ -n \"\$suse_ret2\" ];
...
unset suse_ret0 suse_ret1 suse_ret2 suse_ret3 rsyslog syslog;
";
push(@array_pre_flag, 22);$pre_cmd{25} = "echo \"ip_forward=\"`sysctl -n net.ipv4.ip_forward`
";
push(@array_pre_flag, 25);$pre_cmd{24} = "ssh_banner=`cat /etc/ssh/sshd_config | grep -v '^[[:space:]]*#' | grep -i Banner|awk '{print \$2}'`;
...
";
push(@array_pre_flag, 24);$pre_cmd{27} = "cat /etc/inittab|grep -v \"^#\"|grep \"ctrlaltdel\"
";
push(@array_pre_flag, 27);$pre_cmd{26} = "ps -ef |egrep \"nfs|rpc.mountd|rpc.nfsd|rpc.lockd|rpc.statd\"|egrep -v \"egrep|grep\"|grep -v kblockd
...
";
push(@array_pre_flag, 26);$pre_cmd{29} = "awk -F: '( \$2 == \"\" ) { print \$1 }' /etc/shadow
echo \"result=\"`awk -F: '( \$2 == \"\" ) { print \$1 }' /etc/shadow |wc -l`
";
push(@array_pre_flag, 29);$pre_cmd{28} = "cat /etc/hosts.allow |sed '/^#/d'|sed '/^\$/d'|egrep -i \"sshd|telnet|all\"
...
";
push(@array_pre_flag, 28);$pre_cmd{31} = "echo \"accept_redirects=\"`sysctl -n net.ipv4.conf.all.accept_redirects`
";
push(@array_pre_flag, 31);$pre_cmd{30} = "ntpstatus=`ps -ef|egrep \"ntp|ntpd\"|grep -v grep|wc -l`
if [ \$ntpstatus != 0 ];
then
echo \"ntp:start\"
...
";
push(@array_pre_flag, 30);$pre_cmd{34} = "up_uidmin=`(grep -v ^# /etc/login.defs |grep \"^UID_MIN\"|awk '(\$1=\"UID_MIN\"){print \$2}')`
...
";
push(@array_pre_flag, 35);$pre_cmd{33} = "SNMPD_STATUS=`ps -ef|grep snmpd|egrep -v \"grep\"|wc -l`;
Check_SNMPD ()
{
...
";
push(@array_pre_flag, 33);$pre_cmd{38} = "uname -a
if [ -f /etc/SuSE-release ];
then
...
then red_ret=`cat /etc/syslog.conf | grep -v \"^[[:space:]]*#\" | egrep \"auth\\.\\*[[:space:]]*\\/.*|auth\\.info[[:space:]]*\\/.*|authpriv\\.\\*[[:space:]]*\\/.*|authpriv\\.info[[:space:]]*\\/.*\"`
fi
if [ -s /etc/rsyslog.conf ];
then red_ret2=`cat /etc/rsyslog.conf | grep -v \"^[[:space:]]*#\" | egrep \"auth\\.\\*[[:space:]]*|auth\\.info[[:space:]]*|authpriv\\.\\*[[:space:]]*|authpriv\\.info[[:space:]]*\"`
fi
if [ -s /etc/syslog-ng/syslog-ng.conf ];
then
...
";
push(@array_pre_flag, 39);$pre_cmd{36} = "FTPSTATUS=`ps -ef|grep ftpd|grep -v grep|wc -l`;
Check_ftp ()
{
...
";
push(@array_pre_flag, 37);$pre_cmd{42} = "FTPSTATUS=`ps -ef|grep ftpd|grep -v \"grep\"|egrep -v \"tftpd\"|wc -l`;
echo \$FTPSTATUS;
if [ \$FTPSTATUS -eq 0 ];
then  echo \"FTP is not running.FTP check result:true\";
fi;
...
if [ \$FTPSTATUS -ge 1 ];
then  Checkftpt ;
else  echo \"FTP is not running.FTP check result:true\";
fi
echo \$FTPSTATUS;
...
unset red_ret suse_ret suse_ret2 suse_ret3;
";
push(@array_pre_flag, 43);$pre_cmd{40} = "unset syslogd_status syslogng_status rsyslogd_status LOGDIR;
...
unset suse_version FILE;
";
push(@array_pre_flag, 41);$pre_cmd{46} = "cat /etc/pam.d/su|grep -v \"^[[:space:]]*#\"|grep -v \"^\$\"|grep \"^auth\"
cat /etc/*-release
";
push(@array_pre_flag, 46);$pre_cmd{47} = "if [ `ps -ef|grep ftpd|grep -v \"grep\"|egrep -v \"tftpd\"|wc -l` -ge 1 ];
then
if [ -f /etc/vsftpd.conf ];
then
...
then
Anonymous_1=`cat /usr/local/etc/proftpd.conf|sed '/^\\s*#/d'|sed '/^\\s*\$/d'|sed -n '/<Anonymous.*/,/<\\/Anonymous>/p'|grep -i \"AnonRequirePassword[[:space:]]*on\"|wc -l`;
Anonymous_2=`cat /usr/local/etc/proftpd.conf|sed '/^\\s*#/d'|sed '/^\\s*\$/d'|sed -n '/<Anonymous.*/,/<\\/Anonymous>/p'|egrep -i \"User|Group|UserAlias\"|wc -l`;
if ([ \$Anonymous_1 -ge 1 ] || [ \$Anonymous_2 -lt 3 ])
then
echo \"proftp There is no anonymous logins\";
else
...
";
push(@array_pre_flag, 47);$pre_cmd{44} = "FTPSTATUS=`ps -ef|grep ftpd|grep -v \"grep\"|egrep -v \"tftpd\"|wc -l`
Check_ftp ()
{
if [ -f /etc/vsftpd.conf ];
then
...
unset FTPSTATUS;
";
push(@array_pre_flag, 44);$pre_cmd{10011} = "ldd --version
rpm -q --changelog glibc|grep CVE
";
push(@array_pre_flag, 10011);$pre_cmd{4597} = "env -i  X='() { (a)=>\\' bash -c '/dev/stdout echo vulnerable'  2>/dev/null
env x='() { :;}; echo vulnerable' bash -c \"echo this is a test\"
env X='() { (a)=>\\' bash -c \"echo echo vuln\"; [[ \"\$(cat echo)\" == \"vuln\" ]] && echo \"vulnerable\"
cat /etc/redhat-release
bash --version
";
push(@array_pre_flag, 4597);$pre_cmd{4596} = "openssl version
";
push(@array_pre_flag, 4596);$pre_cmd{10015} = "com=`cat /etc/shadow`
if ( [ \"\$com\" ] )
then
echo \"\$com\"
fi
";
push(@array_pre_flag, 10015);$pre_cmd{10016} = "netstat -ntl
netstat -nul
";
push(@array_pre_flag, 10016);$pre_cmd{10042} = "ApachePro=`ps -ef|grep httpd|grep -v \"grep\"|wc -l`
checkApache()
{
...
echo \"Apache is running.Apac:true\"
checkApache;
ApacheVer=`apachectl -v|grep -i \"version\" | awk '{print \$3}'`
if ([ ! \"\$ApacheVer\" ])
then
echo \"ApacheVer is null\"
else
echo \"ApaVer=\$ApacheVer\"
fi
fi;

TomcatPro=`ps -ef|grep tomcat|grep -v \"grep\"|wc -l`
checkTomcat()
{
echo TomcatBin=`ps -ef |grep java |grep -v \"grep\" |awk -Fclasspath '{print \$2}' |awk -Fbootstrap.jar '{print \$1}'`
...
echo \"Tomcat is running.Tom:true\"
checkTomcat;
TomcatBin=`ps -ef |grep java |grep -v \"grep\" |awk -Fclasspath '{print \$2}' |awk -F/bootstrap.jar '{print \$1}'`
fi;

JbossPro=`ps -ef|grep jboss|grep -v \"grep\"|wc -l`
checkJboss()
{
echo \$CLASSPATH
echo \$JBOSS_HOME
echo JbossPro=`ps -ef|grep jboss|grep -v \"grep\"`
}
...
echo \"Jboss is running.Jbo:true\"
checkJboss;
fi;

WeblogicPro=`ps -ef |grep weblogic|grep -v \"grep\"|wc -l`
checkWeblogicPro()
{
echo WeblogicPro=`ps -ef|grep weblogic|grep -v \"grep\"`

}

if ([ \$WeblogicPro -eq 0 ])
then
echo \"WeblogicPro is not running.\"
else
echo \"WeblogicPro is running.Weblog:true\"
checkWeblogicPro;
fi;
";
push(@array_pre_flag, 10042);$pre_cmd{10040} = "echo IP
echo `/sbin/ifconfig -a|grep inet|grep -v 127.0.0.1 |grep -v inet6`
";
push(@array_pre_flag, 10040);$pre_cmd{10041} = "OraclePro=`ps -ef|grep tnslsnr|grep -v \"grep\"|wc -l`
checkOracle()
{
echo OraclePro=`ps -ef|grep tnslsnr|grep -v \"grep\"`
echo OraclePort=`netstat -ntl |grep 1521 |grep \"LISTEN\"`
}
if ( [ \$OraclePro -eq 0 ] )
then
echo \"Oracle is not running\"
else
echo \"Oracle is running.Ora:true\"
checkOracle;
fi;

MysqlPro=`ps -ef|grep mysql|grep -v \"grep\"|wc -l`
checkMysql()
{
echo MysqlPro=`ps -ef|grep mysql|grep -v \"grep\"`
echo MysqlPort=`netstat -ntl |grep 3306 |grep \"LISTEN\"`
}
if ( [ \$MysqlPro -eq 0 ] )
then
echo \"Mysql is not running\"
else
echo \"Mysql is running.Mysq:true\"
checkMysql;
fi;

MongodbPro=`ps -ef|grep mongodb|grep -v \"grep\"|wc -l`
checkMongodb()
{
echo MongodbPro=`ps -ef|grep mongodb|grep -v \"grep\"`
echo MongodbPort=`netstat -ntl |grep 27017 |grep \"LISTEN\"`
}
if ( [ \$MongodbPro -eq 0 ] )
then
echo \"Mongodb is not running\"
else
echo \"Mongodb is running.Mongo:true\"
checkMongodb;
fi;
";
push(@array_pre_flag, 10041);$pre_cmd{32} = "FTPSTATUS=`ps -ef|grep ftpd|grep -v \"grep\"|egrep -v \"tftpd\"|wc -l`
checkftp()
{
if [ -f /etc/vsftpd.conf ];
then
...
if [ -f /etc/pure-ftpd.conf ];
then
PUFTPCONF=\"/etc/pure-ftpd.conf\";
else
if [ -f /etc/pure-ftpd/pure-ftpd.conf ];
then
PUFTPCONF=\"/etc/pure-ftpd/pure-ftpd.conf\";
fi;
fi;
if [ -f \"\$PUFTPCONF\" ];
then
if [ `cat \$PUFTPCONF|grep -v \"^[[:space:]]*#\"|grep -v \"^[[:space:]]*\$\"|grep -i \"FortunesFile\"|wc -l` -eq 0 ];
then
echo \"pure-ftpd is running.banner in pure-ftpd.conf is not recommended.FTP check result:false.\";
else
if [ -s \"`cat \$PUFTPCONF|grep -v \"^[[:space:]]*#\"|grep -v \"^[[:space:]]*\$\"|grep -i \"FortunesFile\"|awk '{print \$2}'`\" ];
then
...
if [ -f /etc/ftpaccess ];
then
AFTPCONF=\"/etc/ftpaccess\";
else
if [ -f /etc/ftpd/ftpaccess ];
then
AFTPCONF=\"/etc/ftpd/ftpaccess\";
fi;
fi;
if [ -f \"\$AFTPCONF\" ];
then
if [ `cat \$AFTPCONF|grep -v \"^[[:space:]]*#\"|grep -v \"^[[:space:]]*\$\"|grep -i \"^[[:space:]]*banner\"|wc -l` -eq 0 ];
then
echo \"wu-ftpd is running.banner in wu-ftpd.conf is not recommended.FTP check result:false.\";
else
if [ -s \"`cat \$AFTPCONF|grep -v \"^[[:space:]]*#\"|grep -v \"^[[:space:]]*\$\"|grep -i \"^[[:space:]]*banner\"|awk '{print \$2}'`\" ];
then
...
if [ -f /etc/proftpd.conf ];
then
PFTPCONF=\"/etc/proftpd.conf\";
else
if [ -f /etc/proftpd/proftpd.conf ];
then
PFTPCONF=\"/etc/proftpd/proftpd.conf\";
else
if [ -f /usr/local/etc/proftpd.conf ];
then
PFTPCONF=\"/usr/local/etc/proftpd.conf\";
else
if [ -f /usr/local/proftpd/etc/proftpd.conf ];
then
PFTPCONF=\"/usr/local/proftpd/etc/proftpd.conf\";
fi;
fi;
fi;
fi;
if [ -f \"\$PFTPCONF\" ];
then
if [ -s \"`cat \$PFTPCONF|sed '/^\\s*#/d'|sed '/^\\s*\$/d'|sed '/<Anonymous.*>/,/<\\/Anonymous>/d'|grep -i \"DisplayConnect\"|awk '{print \$2}'`\" ];
then
...
unset FTPSTATUS;
";
push(@array_pre_flag, 32);$pre_cmd{10035} = "Cpu=\$(cat /proc/stat 2>/dev/null | grep 'cpu ' | awk '{print \$2\" \"\$3\" \"\$4\" \"\$5\" \"\$6\" \"\$7\" \"\$8}')
echo \$Cpu
Total=\$(echo \$Cpu | awk '{print \$1+\$2+\$3+\$4}')
echo \$Total
Used=\$(echo \$Cpu | awk '{print \$1+\$2+\$3}')
echo \$Used
Used_Rate=`expr  \$Used/\$Total*100 | bc -l`
Cpu_Used_Rate=`expr  \$Used_Rate/1 | bc`
echo Cpu_Used_Rate=\$Cpu_Used_Rate%
";
push(@array_pre_flag, 10035);$pre_cmd{10039} = "echo Ver
linux_version=`lsb_release -a  2>/dev/null|grep -i \"Description:\"|gawk -F: '{print \$2}'`
if ( [ \"\$linux_version\" ] )
then
echo \"Version=\$linux_version\"
fi
";
push(@array_pre_flag, 10039);$pre_cmd{10034} = "LoadMemory=\$(cat /proc/meminfo | awk '{print \$2}')
...
echo Memory_Used_Rate=\$Memory_Used_Rate%
";
push(@array_pre_flag, 10034);
